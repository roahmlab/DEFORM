...
def train(DLO_type, train_set_number, eval_set_number, train_time_horizon, eval_time_horizon, batch, DEFORM_func, DEFORM_sim, device):
    ...
    if DLO_type == "DLO1":
        ...
        DEFORM_sim.rest_vert = nn.Parameter(rest_vert)
        DEFORM_sim.DEFORM_func.bend_stiffness = nn.Parameter(8e-4 * torch.ones((1, n_edge), device=device))
        DEFORM_sim.DEFORM_func.twist_stiffness = nn.Parameter(0.0e-5 * torch.ones((1, n_edge), device=device))
...
if __name__ == "__main__":
    train(DLO_type="DLO5", train_set_number=56, eval_set_number=14, train_time_horizon=100, eval_time_horizon=500, batch=32, DEFORM_func=DEFORM_func, DEFORM_sim=DEFORM_sim, device="cuda:0")
    # batch = 32
