...
def train(DLO_type, train_set_number, eval_set_number, train_time_horizon, eval_time_horizon, batch, DEFORM_func, DEFORM_sim, device):
    ...
    if DLO_type == "DLO1":
        ...
        DEFORM_sim.m_restEdgeL, DEFORM_sim.m_restRegionL = computeLengths(computeEdges(rest_vert.clone()))
        # DEFORM_sim.load_state_dict(torch.load("save_model/DLO2_50000.pth"))
    ...
    for epoch in range(train_epoch):
        ...
        for data in bar:
            if save_steps % evaluate_period == 0:
                        ...
                        for traj_num in range(eval_target_vertices.size()[1]):
                            with torch.no_grad():
                                if traj_num == 0:
                                    ...
                                    # ax.set_zlim(0, 1.)
                                    # plt.legend()
                                    # plt.savefig('verification_images/%s.png' % (traj_num))
                ...
                save_pickle(eval_losses, "loss_record/eval_loss_DLO2.pkl")
                save_pickle(eval_epochs, "loss_record/eval_epoch_DLO2.pkl")
            ...
            if train_time_horizon > 1:
                ...
                if train_time_horizon == 2:
                    ...
                    epochs.append(update_steps)
                    if save_steps % save_period == 0:
                        save_pickle(losses, "loss_record/train_loss_DLO2.pkl" )
                        save_pickle(epochs, "loss_record/train_epoch_DLO2.pkl")
