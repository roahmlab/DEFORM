...
class DEFORM_sim(nn.Module):
    ...
    def Rod_Init(self, batch, init_direction, m_restEdgeL, clamped_index):
        ...
        m_u0 = self.DEFORM_func.compute_m_u0(m_edges[:, 0], init_direction[:, 0])
    ...
    def Internal_Force(self, m_edges, clamped_index, m_restEdgeL, m_restRegionL, m_kb, m_restWprev, m_restWnext, theta_full, m_m1, m_m2):
        ...
        n = self.n_edge - 1
        dEdQn = self.DEFORM_func.computedEdQj(n, m_kb, m_m1[:, n], m_m2[:, n], m_theta, J * torch.clamp(self.DEFORM_func.bend_stiffness[:, n].unsqueeze(-1), self.DEFORM_func.stiff_threshold), m_restWprev, m_restWnext, m_restRegionL)
        for i in range(self.n_vert):
            ...
            o_forces[:, i] += dEdQn.unsqueeze(dim=1) * GH
        o_forces = torch.where(torch.norm(o_forces, dim=2).unsqueeze(dim=-1) >= self.DEFORM_func.force_scale.repeat(1, self.n_vert).unsqueeze(dim=-1), F.normalize(o_forces, dim=2) * self.DEFORM_func.force_scale.unsqueeze(dim=-1), o_forces)
        return o_forces
    ...
    def Internal_Force_Vectorize(self, m_edges, clamped_index, m_restEdgeL, m_restRegionL, m_kb, m_restWprev, m_restWnext, theta_full, m_m1, m_m2):
        ...
        o_forces += b_GHn[:, :, -1] * dEdQn.unsqueeze(dim=1).unsqueeze(dim=1)
        o_forces = torch.where(torch.norm(o_forces, dim=2).unsqueeze(dim=-1) >= self.DEFORM_func.force_scale.repeat(1, self.n_vert).unsqueeze(dim=-1), F.normalize(o_forces, dim=2) * self.DEFORM_func.force_scale.unsqueeze(dim=-1), o_forces)
    ...
    def forward(self, current_vert, current_v, init_direction, clamped_index, m_u0, input, clamped_selection, theta_full, mode="train"):
        if mode == "train":
            ...
            theta_full, material_m1, material_m2, m_kb = self.DEFORM_func.updateCurrentState(current_vert, m_u0, clamped_index, m_restEdgeL, m_restWprev, m_restWnext, m_restRegionL, control_theta, theta_full)
            ...
            """residual"""
            current_vert[:, 2:-2] = current_vert[:, 2:-2] + x_dot * self.dt
        ...
        if mode == "evaluation":
            ...
            m_restEdgeL, m_restRegionL = self.m_restEdgeL.repeat(batch, 1), self.m_restRegionL.repeat(batch, 1)
            control_theta = torch.zeros(batch, 2, 1).to(self.device)
            theta_full, material_m1, material_m2, m_kb = self.DEFORM_func.updateCurrentState(current_vert, m_u0, clamped_index, m_restEdgeL, self.m_restWprev, self.m_restWnext, m_restRegionL, control_theta, theta_full)
        ...
        if mode == "evaluation_numpy":
            ...
            m_restEdgeL, m_restRegionL = self.m_restEdgeL.repeat(batch, 1), self.m_restRegionL.repeat(batch, 1)
            control_theta = torch.zeros(batch, 2, 1).to(self.device)
            theta_full, material_m1, material_m2, m_kb = self.DEFORM_func.updateCurrentState_numpy(current_vert, m_u0, clamped_index, m_restEdgeL, self.m_restWprev, self.m_restWnext, m_restRegionL, control_theta, theta_full)
            Internal_force = self.Internal_Force_Vectorize(current_edges, clamped_index, m_restEdgeL, m_restRegionL, m_kb, self.m_restWprev, self.m_restWnext, theta_full, material_m1, material_m2)
            Total_force = self.External_Force(self.learned_pmass, current_v.clone(), Internal_force, clamped_index)
            """update"""
            current_vert, delta_vert = self.Integrate_Centerline(current_vert, current_v, Total_force, self.learned_pmass)
