        ...
        for rope_data in bar:
            ...
            for i in range(len(rope_verts) - 1 - 1):
                if i == 0:
                    ...
                    m_u0 = compute_m_u0(rest_edges.float()[:, 0], init_direction.repeat(1, 1, 1)[:, 0])
                    mu_0_list[i] = m_u0
    ...
    for epoch in range(train_epoch):
        ...
        for data in bar:
            if save_steps % evaluate_period == 0:
                ...
                evaluation: with torch, provides gradient
                evaluation with numpy: part of the code is written in numpy; also only compatible when set eval_batch = 1
                """
                with torch.no_grad():
                    eval_time = 0
                    for eval_data in eval_bar:
                        ...
                        DEFORM_sim.m_restWprev, DEFORM_sim.m_restWnext, DEFORM_sim.learned_pmass = DEFORM_sim.Rod_Init(eval_batch, init_direction.repeat(eval_batch, 1, 1), DEFORM_sim.m_restEdgeL.repeat(eval_batch, 1), clamped_index)
                        for traj_num in range(eval_target_vertices.size()[1]):
                            with torch.no_grad():
                                if traj_num == 0:
                                    rest_edges = computeEdges(eval_vertices[:, traj_num])
                                    m_u0 = DEFORM_func.compute_m_u0(rest_edges[:, 0].float(), init_direction.repeat(eval_batch, 1, 1)[:, 0])
                                    current_v = (eval_vertices[:, traj_num] - eval_previous_vertices[:, traj_num]).div(DEFORM_sim.dt)
                                    init_pred_vert_0, current_v, theta_full = DEFORM_sim(eval_vertices[:, traj_num], current_v, init_direction.repeat(eval_batch, 1, 1), clamped_index, m_u0, inputs[:, traj_num], clamped_selection, theta_full, mode="evaluation_numpy")
                                ...
                                if traj_num == 1:
                                    ...
                                    pred_vert, current_v, theta_full = DEFORM_sim(init_pred_vert_0, current_v, init_direction.repeat(eval_batch, 1, 1), clamped_index, m_u0, inputs[:, traj_num], clamped_selection, theta_full, mode="evaluation_numpy")
                                    ...
                                    # vis_gt_vert = torch.Tensor.numpy(eval_target_vertices[:
                                    # fig = plt.figure()
                                    # ax = fig.add_subplot(111, projection='3d')
                                    # # ax.scatter(X_obs, Y_obs, Z_obs, label='Obstacle', s=4, c='orange')
                                ...
                                if traj_num >= 2:
                                    ...
                                    m_u0 = DEFORM_func.parallelTransportFrame(previous_edge[:, 0], current_edges[:, 0],m_u0)
                                    """clone for test"""
                                    # test_current_v = current_v.clone()
                                    # test_theta_full = theta_full.clone()
                                    pred_vert, current_v, theta_full = DEFORM_sim(vert, current_v, init_direction.repeat(eval_batch, 1, 1),clamped_index, m_u0, inputs[:, traj_num], clamped_selection, theta_full, mode="evaluation_numpy")
                                    # pred_vert_test, current_v_test, theta_full_test = DEFORM_sim(vert, test_current_v, init_direction.repeat(eval_batch, 1, 1),clamped_index, m_u0, inputs[:, traj_num], clamped_selection, test_theta_full, mode="evaluation")
                                    # print(pred_vert_test[0] - pred_vert[0])
            ...
            if train_time_horizon > 1:
                ...
                if train_time_horizon == 2:
                    ...
                    for traj_num in range(train_time_horizon):
                        if traj_num == 0:
                            rest_edges = computeEdges(vertices[:, traj_num])
                            m_restEdgeL, m_restRegionL = computeLengths(rest_edges)
                            current_v = (vertices[:, traj_num] - previous_vertices[:, traj_num]).div(DEFORM_sim.dt)
                            target_v = (target_vertices[:, traj_num] - vertices[:, traj_num]).div(DEFORM_sim.dt)
...
if __name__ == "__main__":
    train(DLO_type="DLO2", train_set_number=56, eval_set_number=14, train_time_horizon=100, eval_time_horizon=500, batch=32, DEFORM_func=DEFORM_func, DEFORM_sim=DEFORM_sim, device="cpu")
    # batch = 32
    # train_dataset = Train_DeformData()
    # eval_dataset = Eval_DeformData()
    ...
    #     n_vert = 13
    #     device = device_swich
    #     DER_func = DER_func(n_vert=n_vert, n_edge=n_vert - 1, device=device)
    #     DEFORM_sim = DEFORM_sim(n_vert=n_vert, n_edge=n_vert-1, pbd_iter=10, device=device)
    #     DEFORM_sim.load_state_dict(torch.load("save_model/thin_DLO_360.pth"))
    #
    # """clamped start and end"""
    # clamped_index = torch.zeros(n_vert)
    # clamped_selection = torch.tensor((0, 1, -2, -1))
    ...
    #     {"params": DEFORM_sim.rest_vert, "lr": 1e-5 * lr_scale},
    #     {"params": DEFORM_sim.mocap_mass, "lr": 1e-5 * lr_scale},
    #     {"params": DEFORM_sim.DER_func.bend_stiffness, "lr": 1e-11 * lr_scale},
    #     {"params": DEFORM_sim.DER_func.twist_stiffness, "lr": 1e-11 * lr_scale},
    #     {"params": DEFORM_sim.vert_conv1.parameters(), "lr": network_lr * lr_scale},
    #     {"params": DEFORM_sim.vert_conv2.parameters(), "lr": network_lr * lr_scale},
    #     {"params": DEFORM_sim.delta_vert_conv1.parameters(), "lr": network_lr * lr_scale},
    #     {"params": DEFORM_sim.delta_vert_conv2.parameters(), "lr": network_lr * lr_scale},
    ...
    #             eval_set, test_set = torch.utils.data.random_split(eval_dataset, [part_eval, eval_data_len - part_eval])
    #             eval_data_loader = DataLoader(eval_set, batch_size=eval_batch, shuffle=True, drop_last=True)
    #             torch.save(DEFORM_sim.state_dict(),os.path.join("save_model/", "thin_DLO_%s.pth" % str(update_steps)))
    #             eval_loss = 0
    #             eval_bar = tqdm(eval_data_loader)
    #
    #             with torch.no_grad():
    ...
    #                     eval_previous_vertices, eval_vertices, eval_target_vertices = eval_data
    #                     inputs = eval_target_vertices[:, :, clamped_selection]
    #                     theta_full = torch.zeros(eval_batch, n_vert - 1).to(device_swich)
    #                     for traj_num in range(eval_target_vertices.size()[1]):
    #                         with torch.no_grad():
    #                             if traj_num == 0:
    #                                 rest_edges = computeEdges(eval_vertices[:, traj_num])
    #                                 m_restEdgeL, m_restRegionL = computeLengths(rest_edges.clone())
    #                                 m_u0 = DER_func.compute_m_u0(rest_edges[:, 0].float(), init_direction.repeat(eval_batch, 1, 1)[:, 0])
    #                                 current_v = (eval_vertices[:, traj_num] - eval_previous_vertices[:, traj_num]).div(DEFORM_sim.dt)
    ...
    #                                 previous_edge = computeEdges(eval_previous_vertices[:, traj_num])
    #                                 current_edges = computeEdges(init_pred_vert_0)
    #                                 m_u0 = DER_func.parallelTransportFrame(previous_edge[:, 0], current_edges[:, 0], m_u0)
    ...
    #             eval_losses.append(eval_loss.cpu().detach().numpy() / (eval_time_horizon * part_eval // eval_batch))
    #             eval_epochs.append(update_steps)
    #             save_pickle(eval_losses, "loss_record/eval_thin_DLO_%s.pkl" % eval_time_horizon)
    #             save_pickle(eval_epochs, "loss_record/step_thin_DLO_%s.pkl" % eval_time_horizon)
    #
    #         theta_full = torch.zeros(batch, n_vert - 1).to(device_swich)
    #         traj_loss_record = 0
    #         # try:
    #         if time_horizon == 1:
    #             previous_vertices, vertices, target_vertices, m_u0 = data
    #             vis_pred_vert = torch.Tensor.numpy(vertices[:, 0].to(device))
    #             vis_gt_vert = torch.Tensor.numpy(target_vertices[:, 0].to(device))
    #
    #             # ax.scatter(X_obs, Y_obs, Z_obs, label='Obstacle', s=4, c='orange')
    #             inputs = target_vertices[:, :, clamped_selection]
    #
    ...
    #             current_v = (vertices[:, traj_num] - previous_vertices[:, traj_num]).div(DEFORM_sim.dt)
    #             target_v = (target_vertices[:, traj_num] - vertices[:, traj_num]).div(DEFORM_sim.dt)
    #             pred_vertice, pred_v, theta_full = DEFORM_sim(vertices[:, traj_num], current_v,
    #                                                        init_direction.repeat(batch, 1, 1), clamped_index,
    #                                                        m_u0[:, traj_num], inputs[:, traj_num],
    #                                                        clamped_selection, theta_full)
    #
    #             traj_loss = loss_func(pred_vertice, target_vertices[:, traj_num])
    #             v_loss = loss_func(pred_v, target_v)
    #             assert torch.isfinite(traj_loss.all()), "infinite problem: material_m2"
    #             assert not torch.isnan(traj_loss).any(), "nan problem: material_m2"
    #
    #             assert torch.isfinite(v_loss.all()), "infinite problem: material_m2"
    #             assert not torch.isnan(v_loss).any(), "nan problem: material_m2"
    #             (traj_loss + v_loss).backward(retain_graph=True)
    #             optimizer.step()
    #
    #             save_steps += 1
    #             update_steps += 1
    #
    #             losses.append(traj_loss.cpu().detach().numpy() / time_horizon)
    #             epochs.append(update_steps)
    #             if save_steps % save_period == 0:
    #                 save_pickle(losses, "loss_record/eval_thin_DLO_%s.pkl" % time_horizon)
    #                 save_pickle(epochs, "loss_record/step_thin_DLO_%s.pkl" % time_horizon)
    #
    #         if time_horizon > 1:
    #             previous_vertices, vertices, target_vertices, m_u0 = data
    #             if time_horizon == 2:
    ...
    #                         current_v = (vertices[:, traj_num] - previous_vertices[:, traj_num]).div(DEFORM_sim.dt)
    #                         target_v = (target_vertices[:, traj_num] - vertices[:, traj_num]).div(DEFORM_sim.dt)
    #                         pred_vertice, current_v, theta_full = DEFORM_sim(vertices[:, traj_num], current_v,
    #                                                                       init_direction.repeat(batch, 1, 1),
    #                                                                       clamped_index, m_u0[:, traj_num],
    #                                                                       inputs[:, traj_num], clamped_selection,
    #                                                                       theta_full)
    #                         traj_loss = loss_func(pred_vertice, target_vertices[:, traj_num])
    #                         v_loss = loss_func(current_v, target_v)
    #                         loss += traj_loss + v_loss
    ...
    #                         previous_edge = computeEdges(previous_vertices[:, traj_num])
    #                         current_edges = computeEdges(pred_vertice)
    #                         m_u0 = DER_func.parallelTransportFrame(previous_edge[:, 0], current_edges[:, 0],
    #                                                                m_u0[:, traj_num])
    #                         target_v = (target_vertices[:, traj_num] - vertices[:, traj_num]).div(DEFORM_sim.dt)
    #                         pred_vertice, current_v, theta_full = DEFORM_sim(
    #                             pred_vertice.sgd_nocontact_fast_step_clone(), current_v.clone(),
    #                             init_direction.repeat(batch, 1, 1), clamped_index, m_u0, inputs[:, traj_num],
    #                             clamped_selection, theta_full)
    #                         traj_loss = loss_func(pred_vertice, target_vertices[:, traj_num])
    #                         v_loss = loss_func(current_v, target_v)
    #                         loss += traj_loss + v_loss
    ...
    #                 optimizer.zero_grad()
    #                 loss = 0
    #                 for traj_num in range(time_horizon):
    #                     if traj_num == 0:
    #                         rest_edges = computeEdges(vertices[:, traj_num])
    #                         m_restEdgeL, m_restRegionL = computeLengths(rest_edges)
    #                         current_v = (vertices[:, traj_num] - previous_vertices[:, traj_num]).div(DEFORM_sim.dt)
    #                         target_v = (target_vertices[:, traj_num] - vertices[:, traj_num]).div(DEFORM_sim.dt)
    #                         pred_vert, current_v, theta_full = DEFORM_sim(vertices[:, traj_num], current_v,
    #                                                                    init_direction.repeat(batch, 1, 1),
    #                                                                    clamped_index, m_u0[:, traj_num],
    #                                                                    inputs[:, traj_num], clamped_selection,
    #                                                                    theta_full)
    ...
    #                                                                m_u0[:, traj_num])
    #                         target_v = (target_vertices[:, traj_num] - vertices[:, traj_num]).div(DEFORM_sim.dt)
    #                         pred_vert, current_v, theta_full = DEFORM_sim(pred_vert.clone(), current_v.clone(),
    #                                                                    init_direction.repeat(batch, 1, 1),
    #                                                                    clamped_index, m_u0, inputs[:, traj_num],
    #                                                                    clamped_selection, theta_full)
    #
    #                         traj_loss = loss_func(pred_vert, target_vertices[:, traj_num])
    #                         v_loss = loss_func(current_v, target_v)
    ...
    #                         m_u0 = DER_func.parallelTransportFrame(previous_edge[:, 0], current_edges[:, 0], m_u0)
    #                         target_v = (target_vertices[:, traj_num] - vertices[:, traj_num]).div(DEFORM_sim.dt)
    #                         pred_vert, current_v, theta_full = DEFORM_sim(vert.clone(), current_v.clone(),
    #                                                                    init_direction.repeat(batch, 1, 1),
    #                                                                    clamped_index, m_u0, inputs[:, traj_num],
    #                                                                    clamped_selection, theta_full)
    #                         traj_loss = loss_func(pred_vert, target_vertices[:, traj_num])
    ...
    #
    #                 # print('List Learning')
    #                 # print(DEFORM_sim.DER_func.bend_stiffness)
    #                 # print(DEFORM_sim.DER_func.twist_stiffness)
    #                 # print(DEFORM_sim.time_effective)
    #                 # print(DEFORM_sim.damping)
    #                 # print(DEFORM_sim.m_pmass)
    #                 loss.backward(retain_graph=True)
    #                 optimizer.step()
    #                 save_steps += 1
    #                 update_steps += 1
    #                 losses.append(traj_loss_record.cpu().detach().numpy() / time_horizon)
    #                 epochs.append(update_steps)
    #                 if save_steps % save_period == 0:
    #                     save_pickle(losses, "loss_record/eval_thin_DLO_%s.pkl" % time_horizon)
    #                     save_pickle(epochs, "loss_record/step_thin_DLO_%s.pkl" % time_horizon)
